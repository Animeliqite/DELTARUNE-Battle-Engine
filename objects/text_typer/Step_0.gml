...
if(_paused&&keyboard_check_pressed(vk_enter)){
	_paused=false;
}
if(_skippable&&!_paused&&keyboard_check_pressed(vk_shift)){
	_skipping=true;
	_sleep=0;
...
if(instance_exists(_inst_face)){
	_inst_face.gui=_gui;
	_inst_face.depth=_depth;
	if(!_sleep&&!_paused&&_char_proc<=string_length(text)){
		if(!(_inst_face._talking)){
			with(_inst_face){
				event_user(0);
			}
		}
